...
GitHub release with the zip attached. The app icon lives in
`assets/AppIcon.icon` (and `AppIcon-Dev.icon`), Icon Composer packages
written by `scripts/GenerateIcon.swift` from the Figma geometry; `bundle.sh`
compiles them with actool so macOS 26 renders the Liquid Glass version and
older releases get a flattened `.icns`.
